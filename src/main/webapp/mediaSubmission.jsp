	...
	wildbook.init( function() {
		updateAllCollections(function() {
			...
			if (mediaSubmissionID) {
				...
				alert('no mediaSubmissionID passed.');
				return;
			}
		});
...
console.log(m);
	...
	for (var i = 0 ; i < m.length ; i++) {
		...
		var imgSrc = mObj.url();
		if (isGeoFile(m[i])) continue;

		var classes = '';
		var tags = getTags(m[i]);
//console.warn('tags! %o', tags);
		if (tags && (tags.length > 0)) {
			if (tags.indexOf('trash') > -1) continue;
/*
...
<div id="work-div">
	<h1 style="text-align: center; margin-top: 20px; background-color: #CCC; padding: 5px; border-radius: 4px;">
Thank you for submitting images!  These files of yours were used:
</h1>
