...
//////////// TODO it seems like this should either (a) throw an exception itself; or (b) return boolean of success.  obviously the latter was disabled(?).  whassup?  -jon 20140619
  ...
  public List<Decision> getDecisionsForEncounter(Encounter enc){
    try {
      ...
      // String queryString = "SELECT \"ID\" FROM DECISION WHERE ENCOUNTER_CATALOGNUMBER_OID = '" + enc.getCatalogNumber() + "'"; //TODO changeMe
      Query query = pm.newQuery(queryString); //"javax.jdo.query.SQL",
      ...
      System.out.println("Exception on getDecisionsForEncounter encounter: " + enc.toString());
      e.printStackTrace();
    }
    return null;
  }
