...
////TODO this will need to be generified for offsite storage prob via SinglePhotoVideo? as in EncounterForm?
        if (part.isFile()) {
          ...
          if (fileName != null) {
						//fileName = UUID.randomUUID().toString() + "-orig." + FilenameUtils.getExtension(fileName);
      ...
      if (myShepherd.isEncounter(encounterNumber)) {
        ...
        try {
          ...
          enc.addSinglePhotoVideo(newSPV);
					enc.refreshAssetFormats(context, ServletUtilities.dataDir(context, rootWebappPath).getAbsolutePath(), newSPV, false);
